...
class ARKDinoSpawnWeightMultipliers(ARKConfigItemHandler):
    def __init__(self):
        super().__init__()
        self._config_item = OrderedDict(
            {'DinoSpawnWeightMultipliers': OrderedDict()})
    ...
    def from_string(self, config_string):
        ...
        multiplier_name = re.search(
            r'DinoNameTag=\"\w+\"',
            config_string).group()
        
        multiplier_name = cleanup_string(multiplier_name)
        multiplier.update(
            {k: v for k, v in [multiplier_name.split('=')]}
            )

        options = re.search(
            r'SpawnWeightMultiplier=.*$',
            config_string).group()
