...
class ARKPerLevelStatsMultiplier_DinoTamed_Add(ARKConfigItemHandler):
    def __init__(self):
        super().__init__()
        self._idx = '0'
        self._config_item = OrderedDict({
            'PerLevelStatsMultiplier_DinoTamed_Add[' + self._idx + ']': str()
        })
    ...
    @config_item.deleter
    def config_item(self):
        super(ARKPerLevelStatsMultiplier_DinoTamed_Add)
    ...
    def config_name(self, value):
        super(ARKPerLevelStatsMultiplier_DinoTamed_Add, value)
    ...
    def from_string(self, config_string):
        self._idx = config_string.split(
            '=')[0].strip(
                'PerLevelStatsMultiplier_DinoTamed_Add').strip(
                    '[').strip(']')
        value = config_string.split('=')[1]
        self._config_item = OrderedDict({
            'PerLevelStatsMultiplier_DinoTamed_Add[' + self._idx + ']': value
        })
        self._config_name = next(iter(self._config_item))
