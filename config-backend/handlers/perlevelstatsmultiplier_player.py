...
class ARKPerLevelStatsMultiplier_Player(ARKConfigItemHandler):
    def __init__(self):
        super().__init__()
        self._idx = '0'
        self._config_item = OrderedDict({
            'PerLevelStatsMultiplier_Player[' + self._idx + ']': str()
        })
    ...
    def config_item(self, config_item):
        super(ARKPerLevelStatsMultiplier_Player, config_item)
    
    @config_item.deleter
    def config_item(self):
        super(ARKPerLevelStatsMultiplier_Player)
    ...
    def config_name(self, value):
        super(ARKPerLevelStatsMultiplier_Player, value)
    ...
    def from_string(self, config_string):
        self._idx = config_string.split(
            '=')[0].strip(
                'PerLevelStatsMultiplier_Player').strip(
                    '[').strip(']')
        value = config_string.split('=')[1]
        self._config_item = OrderedDict({
            'PerLevelStatsMultiplier_Player[' + self._idx + ']': value
        })
        self._config_name = next(iter(self._config_item))
