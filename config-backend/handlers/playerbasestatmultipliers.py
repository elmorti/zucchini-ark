...
class ARKPlayerBaseStatMultipliers(ARKConfigItemHandler):
    def __init__(self):
        super().__init__()
        self._idx = '0'
        self._config_item = OrderedDict({
            'PlayerBaseStatMultipliers[' + self._idx + ']': str()
        })
    ...
    def from_string(self, config_string):
        self._idx = config_string.split(
            '=')[0].strip(
                'PlayerBaseStatMultipliers').strip(
                    '[').strip(']')
        value = config_string.split('=')[1]
        self._config_item = OrderedDict({
            'PlayerBaseStatMultipliers[' + self._idx + ']': value
        })
        self._config_name = next(iter(self._config_item))
